...
\colorbox{softblue}{%
  \parbox{0.94\textwidth}{
    \textbf{方案摘要：}将任务建模为时间有向图上的候选边排序。首先以训练历史构造边频次、时效性、转移、跳跃转移、节点窗口和无标签候选分布特征；随后在正向图与反向图上提取时间加权的低秩表示；最后使用 XGBRanker 学习每个查询行内部 100 个候选节点的相对次序。最终 V69 方案针对两个数据集分别选择特征子集、树数和混合系数，在本地严格时间验证上取得合计 MRR 1.3952170297。
  }}
\end{minipage}
\end{center}
\vfill
...
\begin{enumerate}
  ...
  \item 使用完整训练历史重建全部索引，分块预测两个数据集，逐行归一化并打包结果。
\end{enumerate}
...
V62--V69 & 对 dataset1 采用更丰富的 v65 特征并增至 1200 棵树；dataset2 保持 v61 与 850 棵树；独立确定混合系数。 & 两个数据集的稀疏度和主导信号不同，单一统一配置不如数据集特定配置。最终 V69 是可独立复现、无需外部提交的方案。 \\
V71--V94 & 试验 reciprocal-rank、Borda、RRF、门控和外部结果融合。 & 这些是赛后/附加融合探索，依赖其他提交结果，不纳入审核复现包，也不属于最终独立方案。 \\
\bottomrule
\end{longtable}

最终取舍的逻辑不是简单“堆更多特征”，而是：先用时间验证保证评价可信，再让候选采样贴近测试分布；随后通过图统计和双向 SVD补充表达；最后针对两个数据集分别控制特征复杂度和树模型容量。

\section{最终 V69 配置与结果}
...
\toprule
数据集 & 特征配置 & 树数 & SVD & 混合系数 & 本地 MRR \\
\midrule
dataset1 & v65，167/180 维 & 1200 & 正/反各 32 维 & 0.95 & 0.8456168559 \\
dataset2 & v61，105/180 维 & 850 & 正/反各 32 维 & 1.00 & 0.5496001738 \\
合计 & V69 & --- & --- & --- & \textbf{1.3952170297} \\
\bottomrule
\end{tabularx}
\end{table}

这里的本地 MRR 来自固定随机种子和代码所述时间验证过程，用于复现模型选择逻辑；它不是对隐藏测试标签的计算结果。
...
主随机种子固定为 20260525，NumPy 采样、SVD 和 XGBoost 均使用确定的种子。程序在 GPU 训练异常时重建 CPU 模型继续训练；只有 dataset1 和 dataset2 两个结果文件均存在时才生成 \texttt{result.zip}，降低半成品被误提交的风险。
...
\begin{itemize}
  ...
  \item 最终 V69 不读取其他队伍代码、未公开标注或外部提交预测。
  \item V71--V94 融合实验不属于本复现方案，仓库中不包含这些结果或逻辑。
\end{itemize}
...
文件 & 作用 \\
\midrule
\path{code/main.py} & 数据下载与读取、时间验证、历史索引、双向 SVD、特征构建、XGBRanker 训练、分块预测及结果打包。 \\
\path{code/config.json} & 最终 V69 的固定参数快照和本地验证指标。 \\
\path{code/run.sh} & Ubuntu 环境下一键复现入口。 \\
\path{code/check_environment.py} & Python、Jittor、XGBoost 等依赖版本检查。 \\
\path{records/final_metrics.json} & 最终指标、特征数与输出行数记录。 \\
\path{records/reproduction.log} & 完整训练过程中的阶段日志。 \\
...
\begin{itemize}
  ...
  \item \textbf{输出 ZIP 未生成：}确认两个数据集 CSV 都已生成；代码不会为单个数据集自动打包最终结果。
  \item \textbf{依赖版本告警：}优先使用仓库中固定的 \path{requirements.txt}，尤其是 XGBoost 2.1.4。
\end{itemize}

\section{结论}

最终方案的核心并非某一个单独模型，而是四层逻辑的闭环：严格时间验证保证优化方向可信；候选行模拟使本地训练和正式评测一致；显式图统计与双向低秩表示共同覆盖记忆和泛化；数据集特定的特征筛选与容量控制减少过拟合。V69 因此能够在不依赖外部提交和测试标签的前提下，从原始训练数据完整重建模型和结果文件。
