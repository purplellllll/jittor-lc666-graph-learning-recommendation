...
def selected_ranker_feature_names(no_pos: bool = False, profile: str = "sharp"):
    if profile in {"v42", "v51", "v58", "v59", "v60", "v61", "v65", "v67"}:
        keep_exact = {
            "log_pair_cnt",
            "pair_rec",
            "log_rev_pair_cnt",
        ...
        if profile in {"v51", "v59", "v60", "v61", "v65", "v67"}:
            keep_exact.update(
                {
                    "log_test_window_50",
                    "log_test_window_70",
        ...
        if profile in {"v59", "v60", "v61", "v65", "v67"}:
            keep_exact.update(
                {
                    "svd_dot",
                    "svd_cos",
        ...
        if profile in {"v65", "v67"}:
            ...
            keep_exact.update({f"recent_match_{i}" for i in range(10)})
            keep_exact.update({f"log_trans_cnt_{i}" for i in range(10)})
            keep_exact.update({f"trans_rec_{i}" for i in range(10)})
            keep_exact.update({f"log_rev_trans_cnt_{i}" for i in range(10)})
            keep_exact.update({f"rev_trans_rec_{i}" for i in range(10)})
        if profile == "v67":
            keep_exact.update(
                {
                    "log_test_window_90",
...
def ensure_xgboost():
    try:
        ...
        raise RuntimeError(
            "xgboost is missing; install the pinned audit dependencies with "
...
def train_ranker(train: pd.DataFrame, test: pd.DataFrame, name: str, args):
    if args.ranker_dataset != "both" and name != args.ranker_dataset:
        return None
    log(f"Training XGBoost GPU ranker for {name}")
    if name == "dataset2":
        ranker_base_weights = fixed_best_dataset2_weights()
        fallback_mrr = 0.419911417939297
    else:
        fixed = fixed_best_weights(name)
        ranker_base_weights = fixed[0] if fixed is not None else globals()["base_weights"](name)
        fallback_mrr = fixed[1] if fixed is not None else 0.0
    ...
    if len(fake_test) < 1000:
        ...
        return {
            "use_ranker": False,
            "reason": "too_few_rows",
            "base_weights": ranker_base_weights,
            "validation_mrr": fallback_mrr,
        }
    ...
    x_train, y_train, group_train = build_ranker_matrix(
        history, train_fake, train_true, args.ranker_chunk_rows, ranker_base_weights, feature_idx=feature_idx
    )
    x_eval, y_eval, group_eval = build_ranker_matrix(
        history, eval_fake, eval_true, args.ranker_chunk_rows, ranker_base_weights, feature_idx=feature_idx
    )
...
def solve_dataset(data_dir: Path, output_dir: Path, name: str, args):
    ...
    run_ranker = args.use_ranker and (args.ranker_dataset == "both" or name == args.ranker_dataset)
    if run_ranker:
        try:
            ...
            if name == "dataset2":
                fallback_weights = fixed_best_dataset2_weights()
                fallback_mrr = 0.419911417939297
            else:
                fixed = fixed_best_weights(name)
                fallback_weights = fixed[0] if fixed is not None else globals()["base_weights"](name)
                fallback_mrr = fixed[1] if fixed is not None else 0.0
            ranker_info = {
                "use_ranker": False,
                "reason": str(exc),
    ...
    if args.ranker_eval_only and ranker_info is not None:
        return {
            "dataset": name,
            "rows": int(len(test)),
...
def apply_reproduction_preset(args, dataset_name: str):
    ...
    if dataset_name == "dataset1":
        args.ranker_feature_profile = "v65"
        args.ranker_trees = 1200
        args.ranker_force_alpha = 0.95
    else:
        args.ranker_feature_profile = "v61"
        args.ranker_trees = 850
        args.ranker_force_alpha = 1.0
    args.ranker_dataset = "both"
    args.ranker_candidate_mode = "row"
    args.ranker_eval_candidate_mode = "row"
    args.ranker_no_pos = True
    args.ranker_svd_dim = 32
    args.ranker_svd_iter = 5
    log(
        f"Applied final preset for {dataset_name}: "
        f"profile={args.ranker_feature_profile}, trees={args.ranker_trees}, "
        f"alpha={args.ranker_force_alpha}"
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=Path, default=Path("data/track1"))
    parser.add_argument("--zip_path", type=Path, default=Path("data/track1_data.zip"))
    ...
    parser.add_argument("--ranker_dataset", choices=["dataset1", "dataset2", "both"], default="both")
    parser.add_argument("--only_dataset", choices=["dataset1", "dataset2", "both"], default="both")
    parser.add_argument(
        "--ranker_feature_profile",
        choices=["v42", "v51", "v58", "v59", "v60", "v61", "v65", "v67", "mild", "sharp", "core"],
        default="v61",
    )
    ...
    if args.only_dataset != "both":
        dataset_names = [args.only_dataset]
    elif args.ranker_eval_only:
        dataset_names = ["dataset1", "dataset2"] if args.ranker_dataset == "both" else [args.ranker_dataset]
    else:
        dataset_names = ["dataset1", "dataset2"]
    for name in dataset_names:
        apply_reproduction_preset(args, name)
        metadata["datasets"].append(solve_dataset(data_dir, args.output_dir, name, args))

    if not args.ranker_eval_only and all((args.output_dir / f"{name}.csv").exists() for name in ("dataset1", "dataset2")):
        zip_result(args.output_dir, args.result_zip)
        metadata["result_zip"] = str(args.result_zip)
    elif not args.ranker_eval_only:
        log("A single dataset was requested; result.zip is created only after both dataset CSV files exist")
        metadata["result_zip"] = None
    metadata["finished_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    if args.ranker_eval_only:
        metadata["result_zip"] = None
