...
def build_history(train: pd.DataFrame, test: pd.DataFrame, recent_k: int = RECENT_FEATURE_K):
...
def ranker_feature_names():
    ...
    names.extend(
        [
            ...
            "twohop_pair_max_log",
            "twohop_pair_max_rec",
            "twohop_pair_has_count",
            "twohop_pair_row_norm",
            "twohop_pair_inv_rank",
            "rev_twohop_pair_logsum",
            "rev_twohop_pair_rec_sum",
            "rev_twohop_pair_max_log",
            "rev_twohop_pair_max_rec",
            "rev_twohop_pair_has_count",
            "rev_twohop_pair_row_norm",
            "rev_twohop_pair_inv_rank",
        ]
    )
...
def feature_batch(history: History, src, times, candidates, base_score_weights=None):
    ...
    twohop_pair_logsum = np.zeros((n, m), dtype=np.float32)
    twohop_pair_rec_sum = np.zeros((n, m), dtype=np.float32)
    twohop_pair_max_log = np.zeros((n, m), dtype=np.float32)
    twohop_pair_max_rec = np.zeros((n, m), dtype=np.float32)
    twohop_pair_has_count = np.zeros((n, m), dtype=np.float32)
    rev_twohop_pair_logsum = np.zeros((n, m), dtype=np.float32)
    rev_twohop_pair_rec_sum = np.zeros((n, m), dtype=np.float32)
    rev_twohop_pair_max_log = np.zeros((n, m), dtype=np.float32)
    rev_twohop_pair_max_rec = np.zeros((n, m), dtype=np.float32)
    rev_twohop_pair_has_count = np.zeros((n, m), dtype=np.float32)
    for j in range(min(RECENT_FEATURE_K, recent.shape[1])):
        prev = recent[:, j].astype(np.int64)
        has_prev = prev >= 0
        if not np.any(has_prev):
            continue
        row_mask = has_prev[:, None]
        decay = np.float32(1.0 / (j + 1.0))
        prev_rep = np.repeat(np.maximum(prev, 0), m)

        twohop_query = prev_rep * history.base + cand_flat
        twohop_cnt = lookup(history.pair_keys, history.pair_counts, twohop_query).reshape(n, m)
        twohop_last = lookup_last(history.pair_keys, history.pair_last, twohop_query).reshape(n, m)
        twohop_delta = np.maximum(times_int[:, None] - twohop_last, 0)
        twohop_rec = np.where(twohop_last >= 0, np.exp(-twohop_delta / history.tau), 0.0).astype(np.float32)
        twohop_log = np.where(row_mask, np.log1p(twohop_cnt), 0.0).astype(np.float32)
        twohop_rec = np.where(row_mask, twohop_rec, 0.0).astype(np.float32)
        twohop_pair_logsum += decay * twohop_log
        twohop_pair_rec_sum += decay * twohop_rec
        twohop_pair_max_log = np.maximum(twohop_pair_max_log, twohop_log)
        twohop_pair_max_rec = np.maximum(twohop_pair_max_rec, twohop_rec)
        twohop_pair_has_count += ((twohop_cnt > 0) & row_mask).astype(np.float32)

        rev_twohop_query = cand_flat * history.base + prev_rep
        rev_twohop_cnt = lookup(history.pair_keys, history.pair_counts, rev_twohop_query).reshape(n, m)
        rev_twohop_last = lookup_last(history.pair_keys, history.pair_last, rev_twohop_query).reshape(n, m)
        rev_twohop_delta = np.maximum(times_int[:, None] - rev_twohop_last, 0)
        rev_twohop_rec = np.where(rev_twohop_last >= 0, np.exp(-rev_twohop_delta / history.tau), 0.0).astype(np.float32)
        rev_twohop_log = np.where(row_mask, np.log1p(rev_twohop_cnt), 0.0).astype(np.float32)
        rev_twohop_rec = np.where(row_mask, rev_twohop_rec, 0.0).astype(np.float32)
        rev_twohop_pair_logsum += decay * rev_twohop_log
        rev_twohop_pair_rec_sum += decay * rev_twohop_rec
        rev_twohop_pair_max_log = np.maximum(rev_twohop_pair_max_log, rev_twohop_log)
        rev_twohop_pair_max_rec = np.maximum(rev_twohop_pair_max_rec, rev_twohop_rec)
        rev_twohop_pair_has_count += ((rev_twohop_cnt > 0) & row_mask).astype(np.float32)
    ...
    feats.extend(
        [
            ...
            twohop_pair_max_log,
            twohop_pair_max_rec,
            np.log1p(twohop_pair_has_count),
            normalize_rows(twohop_pair_logsum),
            inverse_rank_feature(twohop_pair_logsum),
            rev_twohop_pair_logsum,
            rev_twohop_pair_rec_sum,
            rev_twohop_pair_max_log,
            rev_twohop_pair_max_rec,
            np.log1p(rev_twohop_pair_has_count),
            normalize_rows(rev_twohop_pair_logsum),
            inverse_rank_feature(rev_twohop_pair_logsum),
        ]
    )
...
def top_ranker_features(model, limit: int = 40):
    try:
    ...
    for key, value in gain.items():
        if not key.startswith("f"):
            continue
        idx = int(key[1:])
        if 0 <= idx < len(RANKER_FEATURE_NAMES):
            rows.append({"feature": RANKER_FEATURE_NAMES[idx], "gain": float(value)})
...
def train_ranker(train: pd.DataFrame, test: pd.DataFrame, name: str, args):
    ...
    return {
        ...
        "top_features": top_ranker_features(model),
        "base_weights": base_weights,
    }
...
def main():
    ...
    parser.add_argument("--ranker_trees", type=int, default=620)
