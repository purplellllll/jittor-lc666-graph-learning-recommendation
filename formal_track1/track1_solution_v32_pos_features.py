...
class History:
    ...
    test_cand_invpos_sum: np.ndarray
    test_cand_best_invpos: np.ndarray
    test_src_cand_keys: np.ndarray
    test_src_cand_counts: np.ndarray
    test_src_cand_invpos_sum: np.ndarray
    test_src_cand_best_invpos: np.ndarray
...
def build_history(train: pd.DataFrame, test: pd.DataFrame, recent_k: int = 5):
    ...
    inv_positions = np.tile(1.0 / (np.arange(cand.shape[1], dtype=np.float32) + 1.0), cand.shape[0])
    valid_flat = (flat_cand >= 0) & (flat_cand < node_size)
    test_cand_invpos_sum = np.bincount(
        flat_cand[valid_flat].astype(np.int64),
        weights=inv_positions[valid_flat],
        minlength=node_size,
    ).astype(np.float32)
    cand_order = np.argsort(flat_cand, kind="mergesort")
    cand_keys_sorted = flat_cand[cand_order]
    cand_inv_sorted = inv_positions[cand_order]
    cand_unique, cand_first = np.unique(cand_keys_sorted, return_index=True)
    cand_best_inv = np.maximum.reduceat(cand_inv_sorted, cand_first)
    test_cand_best_invpos = dense_counts(cand_unique, cand_best_inv.astype(np.float32), node_size)

    test_src_rep = np.repeat(test["src"].to_numpy(np.int64, copy=False), cand.shape[1])
    test_src_cand_raw = test_src_rep * base + flat_cand
    src_cand_order = np.argsort(test_src_cand_raw, kind="mergesort")
    src_cand_sorted = test_src_cand_raw[src_cand_order]
    src_cand_inv_sorted = inv_positions[src_cand_order]
    test_src_cand_keys, src_cand_first, test_src_cand_counts = np.unique(
        src_cand_sorted, return_index=True, return_counts=True
    )
    test_src_cand_invpos_sum = np.add.reduceat(src_cand_inv_sorted, src_cand_first)
    test_src_cand_best_invpos = np.maximum.reduceat(src_cand_inv_sorted, src_cand_first)
    ...
    return History(
        ...
        test_cand_invpos_sum=test_cand_invpos_sum,
        test_cand_best_invpos=test_cand_best_invpos,
        ...
        test_src_cand_invpos_sum=test_src_cand_invpos_sum.astype(np.float32, copy=False),
        test_src_cand_best_invpos=test_src_cand_best_invpos.astype(np.float32, copy=False),
    )
...
def ranker_feature_names():
    ...
    names.extend(
        [
            ...
            "log_test_invpos_sum",
            "test_best_invpos",
            "log_test_src_invpos_sum",
            "test_src_best_invpos",
            "test_invpos_row_norm",
            "test_src_invpos_row_norm",
            "test_invpos_inv_rank",
            "test_src_invpos_inv_rank",
            "dst_count_row_norm",
            "dst_recent_row_norm",
...
def feature_batch(history: History, src, times, candidates, base_score_weights=None):
    ...
    dst_rec = np.where(dst_last >= 0, np.exp(-dst_delta / history.tau), 0.0).astype(np.float32)
    test_freq = history.test_cand_count[valid_cand]
    test_invpos_sum = history.test_cand_invpos_sum[valid_cand]
    test_best_invpos = history.test_cand_best_invpos[valid_cand]
    test_src_cand_freq = lookup(history.test_src_cand_keys, history.test_src_cand_counts, pair_query).reshape(n, m)
    test_src_invpos_sum = lookup(
        history.test_src_cand_keys, history.test_src_cand_invpos_sum, pair_query
    ).reshape(n, m)
    test_src_best_invpos = lookup(
        history.test_src_cand_keys, history.test_src_cand_best_invpos, pair_query
    ).reshape(n, m)
    ...
    log_test_invpos_sum = np.log1p(test_invpos_sum).astype(np.float32)
    log_test_src_invpos_sum = np.log1p(test_src_invpos_sum).astype(np.float32)
    ...
    feats.extend(
        [
            ...
            log_test_invpos_sum,
            test_best_invpos.astype(np.float32),
            log_test_src_invpos_sum,
            test_src_best_invpos.astype(np.float32),
            normalize_rows(log_test_invpos_sum),
            normalize_rows(log_test_src_invpos_sum),
            inverse_rank_feature(log_test_invpos_sum),
            inverse_rank_feature(log_test_src_invpos_sum),
