...
def make_recent_in_by_dst(train: pd.DataFrame, node_size: int, k: int):
    recent = np.full((node_size, k), -1, dtype=np.int32)
    src = train["src"].to_numpy(np.int32, copy=False)
    dst = train["dst"].to_numpy(np.int64, copy=False)
    for s, d in zip(src, dst):
        if 0 <= d < node_size:
            recent[d, 1:] = recent[d, :-1]
            recent[d, 0] = s
    return recent
...
def build_history(train: pd.DataFrame, test: pd.DataFrame, recent_k: int = RECENT_FEATURE_K):
    ...
    recent_in_by_dst = make_recent_in_by_dst(train, node_size, recent_k)
    ...
    return History(
        ...
        recent_in_by_dst=recent_in_by_dst,
        max_time=max_time,
        min_time=min_time,
        tau=tau,
...
def ranker_feature_names():
    ...
    names.extend(
        [
            "motif_out_in_count",
            "motif_out_in_weight",
            "motif_out_in_src_decay",
            "motif_out_in_cand_decay",
            "motif_out_in_has",
            "motif_out_in_row_norm",
            "motif_out_in_inv_rank",
        ]
    )
...
def append_recent_overlap_features(feats, left_recent, right_recent):
    k = min(left_recent.shape[1], right_recent.shape[2])
    left = left_recent[:, :k]
    right = right_recent[:, :, :k]
    common_mask = (
        (left[:, None, :, None] >= 0)
        & (right[:, :, None, :] >= 0)
        & (left[:, None, :, None] == right[:, :, None, :])
    )
    left_decay = (1.0 / (np.arange(k, dtype=np.float32) + 1.0))[None, None, :, None]
    right_decay = (1.0 / (np.arange(k, dtype=np.float32) + 1.0))[None, None, None, :]
    count = common_mask.sum(axis=(2, 3)).astype(np.float32)
    left_weight = (common_mask * left_decay).sum(axis=(2, 3)).astype(np.float32)
    right_weight = (common_mask * right_decay).sum(axis=(2, 3)).astype(np.float32)
    weight = (common_mask * left_decay * right_decay).sum(axis=(2, 3)).astype(np.float32)
    feats.extend(
        [
            np.log1p(count),
            weight,
            ...
            normalize_rows(weight),
            inverse_rank_feature(weight),
        ]
    )
...
def feature_batch(history: History, src, times, candidates, base_score_weights=None):
    ...
    cand_in_recent = history.recent_in_by_dst[valid_cand][:, :, :RECENT_FEATURE_K]
    ...
    append_recent_overlap_features(feats, src_recent, cand_in_recent)
...
def main():
    ...
    parser.add_argument("--ranker_trees", type=int, default=720)
