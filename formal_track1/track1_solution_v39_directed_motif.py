...
def ranker_feature_names():
    ...
    for prefix in ["motif_out_in", "motif_in_in", "motif_in_out"]:
        names.extend(
            [
                f"{prefix}_count",
                f"{prefix}_weight",
...
def feature_batch(history: History, src, times, candidates, base_score_weights=None):
    ...
    src_in_recent = history.recent_in_by_dst[src_safe][:, :RECENT_FEATURE_K]
    ...
    append_recent_overlap_features(feats, src_in_recent, cand_in_recent)
    append_recent_overlap_features(feats, src_in_recent, cand_recent)
