...
def build_history(train: pd.DataFrame, test: pd.DataFrame, recent_k: int = RECENT_FEATURE_K):
    ...
    for q in [0.50, 0.70, 0.85, 0.95, 0.99]:
...
def train_ranker(train: pd.DataFrame, test: pd.DataFrame, name: str, args):
    ...
    history = build_history(hist_train, history_test)
...
def main():
    ...
    parser.add_argument("--ranker_objective", choices=["pairwise", "ndcg", "map"], default="pairwise")
    ...
    parser.add_argument("--ranker_feature_profile", choices=["v42", "v51", "mild", "sharp", "core"], default="mild")
